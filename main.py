...
employee_options = ["-- Select Employee --"] + df["Dropdown"].tolist()
selected_dropdown = st.selectbox("Select Employee (with details):", employee_options)
...
memo_text = st.text_area("Memo Text") if "SF-11" in letter_type or "Sick" in letter_type else ""
...
# === Save Word file ===
def generate_docx(template_path, context, emp_eng_name):
    doc = Document(template_path)
    replace_placeholders(doc, context)
    safe_name = emp_eng_name.replace(" ", "_")
    filename = f"{letter_type.replace(' ', '_')}_{safe_name}_{context['LetterDate'].replace('-', '')}.docx"
...
if st.button("Generate Letter"):
    docx_path = generate_docx(template_files[letter_type], context, selected_row[col_eng_name])
